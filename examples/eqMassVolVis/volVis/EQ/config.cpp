...
bool Config::handleEvent( const eq::ConfigEvent* event )
{
    switch( event->data.type )
    {
        ...
        case eq::Event::CHANNEL_POINTER_BUTTON_RELEASE:
        {
	        ...
	        releaseEvent.button  == eq::PTR_BUTTON1)
            {
                _spinY = releaseEvent.dx*5.f;
                _spinX = releaseEvent.dy*5.f;
        ...
        case eq::Event::CHANNEL_POINTER_MOTION:
        {
            switch( event->data.pointerMotion.buttons )
            {
                ...
                case eq::PTR_BUTTON1 | eq::PTR_BUTTON3:
                    _frameData.moveCamera( .0, .0, .005f * event->data.pointerMotion.dy);
                    return true;

                case eq::PTR_BUTTON3:
                    _frameData.moveCamera( .0005f * event->data.pointerMotion.dx,
                                          -.0005f * event->data.pointerMotion.dy, .0 );
                    return true;
            }
        }
